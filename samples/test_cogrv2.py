...
parser.add_argument('--weight', type=str, default='weights/CoGr/latest_CoGrv2_340000.pth')
...
if __name__ == '__main__':
    ...
    for num in num_grasp:
        ...
        with torch.no_grad():
            ...
            for idx in pbar:
                seq_path, img_f = dataset._data[idx]
                ...
                img_ori = cv2.imread(os.path.join("/home/puzek/sdb/dataset/OCID_grasp", seq_path, "rgb", img_f), cv2.COLOR_BGR2RGB)
                dep_ori = cv2.imread(os.path.join("/home/puzek/sdb/dataset/OCID_grasp", seq_path, "depth", img_f), cv2.IMREAD_UNCHANGED) / 1000.
                dep_ori = 1 - (dep_ori / np.max(dep_ori))


                tgt_f = os.path.join(root_dir, seq_path, "masks", "{}.npz".format(img_f[:-4]))
                if not os.path.exists(os.path.abspath("results/grasps/{:04d}".format(idx))):
                    print("Create new directory")
                    os.makedirs(os.path.abspath("results/grasps/{:04d}".format(idx)))
                    

                rgbd, bboxes, rects, ins_masks, pos_masks, qua_masks, ang_masks, wid_masks = dataset[idx]
                ...
                img, depth, box_p, instance_masks, grasps, pos_masks, ang_masks, wid_masks, ids_p = gr_post_processing(
                    ...
                    target_dir="results/grasps/{:04d}".format(idx)
                )
                ...
                # dataset.show_data(img_ori, dep_ori, box_p, ids_p, instance_masks, grasps=tmp, pos_masks=pos_masks, ang_masks=ang_masks, wid_masks=wid_masks, target_dir="results/grasps/{:04d}".format(idx))
                ...
                # from OCID_class_dict import cls_list
                # target_file = "results/grasps/{:04d}/eval_resutls.txt".format(idx)
                # results = []
                # with open(target_file, 'w') as f:
                #     for idx, item in enumerate(obj_num_count):
                #         if item == 0:
                #             continue
                #         name = cls_list[idx+1]
                #         gt_num = item
                #         dt_num = obj_success_count[idx]
                #         tmp = "{}: {}/{} grasped\n".format(name, dt_num, gt_num)
                #         results.append(tmp)
                #     f.writelines(results)
                # f.close()

            print("test result with {} output grasp candidates".format(num))
            ...
            print(overrall_rate)
            print(np.array(total_obj_success_count).sum() / np.array(total_obj_num_count).sum())
