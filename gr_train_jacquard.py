...
# dataset = dataset = GraspNetDetection(
    ...
    while training:
        ...
        for (images, targets, masks, pos_masks, qua_masks, sin_masks, cos_masks, wid_masks) in data_loader:
            ...
            with timer.counter('for+loss'):
                loss_c, loss_b, loss_m, loss_g_pos, loss_g_qua, loss_g_sin, loss_g_cos, loss_g_wid, loss_s = net(images, targets, masks, pos_masks, qua_masks, sin_masks, cos_masks, wid_masks)

                if cfg.cuda:
                    # use .all_reduce() to get the summed loss from all GPUs
                    all_loss = torch.stack([loss_c, loss_b, loss_m, loss_g_pos, loss_g_qua, loss_g_sin, loss_g_cos, loss_g_wid, loss_s], dim=0)
                    dist.all_reduce(all_loss)

            with timer.counter('backward'):
                loss_total = loss_c + loss_b + loss_m + loss_g_pos + loss_g_qua + loss_g_sin + loss_g_cos + loss_g_wid + loss_s
                optimizer.zero_grad()
                loss_total.backward()
            ...
            if step % 10 == 0 and step != start_step:
                if (not cfg.cuda) or main_gpu:
                    ...
                    l_g_sin = all_loss[5].item() / num_gpu if main_gpu else loss_m.item()
                    l_g_cos = all_loss[6].item() / num_gpu if main_gpu else loss_m.item()
                    l_g_wid = all_loss[7].item() / num_gpu if main_gpu else loss_m.item()
                    l_s = all_loss[8].item() / num_gpu if main_gpu else loss_s.item()
                    ...
                    writer.add_scalar('loss/grasp_ang_sin', l_g_sin, global_step=step)
                    writer.add_scalar('loss/grasp_ang_cos', l_g_cos, global_step=step)
                    ...
                    print(f'[Co:Gr-v2]step: {step} | lr: {cur_lr:.2e} | l_class: {l_c:.3f} | l_box: {l_b:.3f} | '
                          f'l_mask: {l_m:.3f} | l_g_pos: {l_g_pos:.3f} | l_g_qua: {l_g_qua:.3f} | l_g_sin: {l_g_sin:.3f} | l_g_cos: {l_g_cos:.3f} | l_g_wid: {l_g_wid:.3f} | l_semantic: {l_s:.3f} | t_t: {t_t:.3f} | t_d: {t_d:.3f} | '
            ...
            # if args.val_interval > 0 and step % args.val_interval == 0 and eval_dataset is not None:
                if (not cfg.cuda) or main_gpu:
                    val_step = step
                    net.eval()
                    overrall_rate = evaluate_jacquard(net.module, eval_dataset, cfg)
                    net.train()
